...
class IndexableSkiplist:
    ...
    def insert(self, value):
        ...
        for level in xrange(d): 
            newnode.next[level] = chain[level].next[level]
            chain[level].next[level] = newnode

            newnode.width[level] = chain[level].width[level] - steps
            chain[level].width[level] = steps + 1
...
def testSkipList(demo= list('Python'), excluded = list('XZ')): # test the funciton of the skiplist data structure
    ...
    assert [s[i] for i in range(len(s))] == sorted(demo)    # check __getitem__(),excluded=[]):
    ...
    assert len(s) == len(demo)                              # check __len__()
    assert list(s) == sorted(demo)                          # check __iter__()
    assert [s[i] for i in range(len(s))] == sorted(demo)    # check __getitem__()    shuffle(demo)
    for char in demo:
...
def showSkipList(demo= list('SkipList'),):
...
if __name__ == '__main__':
    ...
    demo = list('PythonSkiL')
    exclued = list('X')

    testSkipList(demo, exclued)

    helper()
    
    showSkipList(demo) # build and show the skip list 
